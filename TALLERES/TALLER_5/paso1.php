...
function contarCiudadesPorInicial($arr, $inicial){
    ...
    foreach($arr as $ciudad){
        if(strtoupper($ciudad[0]) === strtoupper($inicial)){
            $contador++;
            array_push($ciudadesGuardadas, $ciudad);
        }
    }
    return [
        "contador" => $contador,
...
// implementacion
$letraInicial = 'B';
$numeroCiudades = contarCiudadesPorInicial($ciudades, $letraInicial);
echo "Número de ciudades que comienzan con la letra '$letraInicial': " . $numeroCiudades["contador"] . "\n";
echo "Ciudades encontradas: " . implode(", ", $numeroCiudades["ciudades"]) . "\n";
