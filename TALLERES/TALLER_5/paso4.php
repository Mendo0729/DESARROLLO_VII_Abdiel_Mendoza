...
function buscarLibros($biblioteca, $termino)
{
    ...
    $resultado = array_filter($biblioteca, function ($libro) use ($termino) {
        return (
            strpos(strtolower($libro['titulo']), $termino) !== false ||
            strpos(strtolower($libro['autor']), $termino) !== false
        );
    });

    if (empty($resultado)){
        echo "El libro que busca no existe en la base de datos\n";
    }
    return $resultado;
}
...
$valor = "Cervantes";
$resultadosBusqueda = buscarLibros($biblioteca, $valor);
echo "Resultados de búsqueda para ".$valor . ":\n";
...
function generarReporteBiblioteca($biblioteca){
    ...
    foreach ($biblioteca as $libro){
        ...
        $genero = $libro['genero'];
        if (!isset($totalLibroGenero[$genero])){
            $totalLibroGenero[$genero] = 0;
        }
        $totalLibroGenero[$genero] += 1;

        $autor = $libro['autor'];
        if (!isset($totalLibroAutor[$autor])){
            $totalLibroAutor[$autor] = 0;
        }
        $totalLibroAutor[$autor] += 1;

        $autorMasLibros = null;
        if (!empty($totalLibroAutor)) {
            $maxLibros = max($totalLibroAutor);
            $autorMasLibros = array_search($maxLibros, $totalLibroAutor);
        }
    ...
    return [
        ...
        "Total libros prestados" => $totalLibrosPrestados,
        "Libros por género" => $totalLibroGenero,
        "Autor con más libros" => $autorMasLibros
    ];
}
...
echo "Total de libros: {$reporte['Total libros']}\n";
echo "Total de libros prestados: {$reporte['Total libros prestados']}\n\n";

echo "Libros por género:\n";
foreach ($reporte['Libros por género'] as $genero => $cantidad) {
    echo "  - $genero: $cantidad\n";
}

echo "\nAutor con más libros: {$reporte['Autor con más libros']}\n";
